...
class UserRemoteDatasourceImpl implements UserRemoteDataSource {
  ...
  Future<ICareUser> getUserData() async {
    final DocumentSnapshot<Map<String, dynamic>> documentSnapshot =
        await accessUsersCollection().doc(Constants.uId).get();
  ...
  Future<void> _addUserToFollowing(ICareUser user) async {
    return await accessUserFollowingCollection(Constants.uId!)
        .doc(user.uId)
        .set(user.toJson());
  }
  ...
  Future<void> signOut() async {
    await getIt.get<CacheHelper>().clearPreferencesExcept(
      <KeyAndType>[
        ...
        KeyAndType('${AppStrings.cachedRandomTip}${Constants.uId}', 'String'),
        KeyAndType(
            '${AppStrings.hasUserNotifiedAboutEnhancing}${Constants.uId}',
            'bool'),
      ],
    );
