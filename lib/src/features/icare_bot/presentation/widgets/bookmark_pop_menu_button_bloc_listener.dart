...
class BookmarkPopUpMenuButtonBlocListener extends StatelessWidget {
  const BookmarkPopUpMenuButtonBlocListener({
    super.key,
  ...
  Widget build(BuildContext context) {
    return BlocListener<BookmarkCubit, BookmarkState>(
      listenWhen: (_, current) =>
          current is BookmarkICareBotMessageSuccess ||
          current is BookmarkICareBotMessageError,
      listener: (context, state) {
        context
            .read<BookmarkCubit>()
            .handleBookmarkMessageStates(state, context);
      },
