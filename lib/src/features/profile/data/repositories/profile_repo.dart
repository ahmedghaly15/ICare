...
class ProfileRepo {
  ...
  Future<FirebaseRequestResult<List<TinyTale>>> getUserTinyTales(String uId) {
  ...
  Future<FirebaseRequestResult<List<Photo>>> getUserPhotos(String uId) {
    return executeAndHandleFirebaseErrors<List<Photo>>(
      () async {
        final QuerySnapshot<Map<String, dynamic>> querySnapshot =
            await _profileRemoteDatasource.getUserPhotos(uId);
